...
module.exports=async(req,res)=>{
 try{
  ...
  for(let hour=8;hour<=17&&hour+hours<=21;hour++){const start=new Date(date+`T${String(hour).padStart(2,"0")}:00:00+03:00`),end=new Date(start.getTime()+hours*3600000),busy=events.filter(event=>{const assigned=event.extendedProperties?.private?.employee;return overlaps(event,start,end)&&(!assigned||assigned===employee)}).length,available=busy?0:1;if(available>0)slots.push({start:start.toISOString(),label:start.toLocaleTimeString("en-KE",{hour:"numeric",minute:"2-digit",timeZone:"Africa/Nairobi"}),available})}
