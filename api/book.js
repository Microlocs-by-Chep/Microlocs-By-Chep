...
module.exports=async(req,res)=>{
 ...
 try{
  const {name,phone,notes="",service,duration,start}=req.body||{},hours=Number(duration);
  if(!name||!phone||!["Retie","Installation"].includes(service)||![3,6].includes(hours)||!start)return res.status(400).json({error:"Complete all booking details"});
