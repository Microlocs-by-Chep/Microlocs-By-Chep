...
module.exports=async(req,res)=>{
 ...
 try{
  const {name,phone,notes="",service,employee,duration,start,hairLength="",hairVolume="",extensionInches="",colour=""}=req.body||{},hours=Number(duration);
  if(!name||!phone||!["Retie","Installation","Extensions","Colouring","Consultation"].includes(service)||!["Bree","Joan","Chep"].includes(employee)||![1,3,6].includes(hours)||!start)return res.status(400).json({error:"Complete all booking details"});
